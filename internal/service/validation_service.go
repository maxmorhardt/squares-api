...
type ValidationService interface {
	...
	ValidateSquareUpdate(ctx context.Context, squareID uuid.UUID, req *model.UpdateSquareRequest, user string) (*model.Square, error)
	ValidateSquareClear(ctx context.Context, squareID uuid.UUID, user string) (*model.Square, error)
...
func (s *validationService) ValidateSquareUpdate(ctx context.Context, squareID uuid.UUID, req *model.UpdateSquareRequest, user string) (*model.Square, error) {
	log := util.LoggerFromContext(ctx)

	square, err := s.contestRepo.GetSquareByID(ctx, squareID)
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
		...
		log.Error("failed to get square", "square_id", squareID, "error", err)
		return nil, ErrDatabaseUnavailable
	}
	...
	if !isValidSquareValue(req.Value) {
		...
		return nil, errInvalidSquareValue
	}

	return square, nil
}

func (s *validationService) ValidateSquareClear(ctx context.Context, squareID uuid.UUID, user string) (*model.Square, error) {
	log := util.LoggerFromContext(ctx)
	...
	contest, err := s.contestRepo.GetByID(ctx, square.ContestID)
	if err != nil {
		log.Error("failed to get contest for square clear authorization", "contest_id", square.ContestID, "error", err)
		return nil, ErrDatabaseUnavailable
	}
